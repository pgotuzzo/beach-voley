...
void setOption(Config *config, Option option) {
    if (option.name == MAPPER_OPT_DEBUG) {
        config->tournamentConfig.debugEnabled = true;
    } else if (option.name == MAPPER_OPT_ROWS) {
        config->tournamentConfig.rows = static_cast<unsigned int>(stoi(option.params.front()));
    } else if (option.name == MAPPER_OPT_COLUMNS) {
        config->tournamentConfig.columns = static_cast<unsigned int>(stoi(option.params.front()));
    } else if (option.name == MAPPER_OPT_CAPACITY) {
        config->tournamentConfig.stadiumCapacity = static_cast<unsigned int>(stoi(option.params.front()));
    } else if (option.name == MAPPER_OPT_MATCHES_COUNT) {
        config->tournamentConfig.maxMatches = static_cast<unsigned int>(stoi(option.params.front()));
    } else if (option.name == MAPPER_OPT_PLAYER) {
        config->tournamentConfig.vPlayerNames = option.params;
    } else if (option.name == MAPPER_OPT_PLAYER_N) {
        ...
        config->tournamentConfig.vPlayerNames = players;
    }
}
...
void validConfig(Config config) throw(InvalidConfigException) {
    ...
    if (config.tournamentConfig.maxMatches > config.tournamentConfig.vPlayerNames.size()) {
...
Config parseConfig(vector<string> arguments) throw(ParseException) {
    ...
    tournamentConfig.debugEnabled = false;
